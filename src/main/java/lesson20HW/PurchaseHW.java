...
public class PurchaseHW {
    ...
    // static позвол вызвать метод класса. Без статика этот метод вызывается только у объекта класса.
    public static void addPurchaseInList(ArrayList<PurchaseHW> list, String name, double price) {
        list.add(new PurchaseHW(name, price)); // добавляем в лист созданный объект класса Purchase  и передаем для конструктора параметр name & price
    ...
    public static void searchPurchase(ArrayList<PurchaseHW> list, String name) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (name.equals(list.get(i).getItemName())) { // Сравниваем входящее имя с: 1. объектами (вытаскиваем коробку под индексом i, и потом вытаксиваем имя из коробки.
